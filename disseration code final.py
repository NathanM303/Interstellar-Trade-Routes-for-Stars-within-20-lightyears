# -*- coding: utf-8 -*-
"""
Created on Thu Nov 21 15:55:05 2024
...
binary_separation_distance = 0.07
"""To provide manual input:
float(input("What is your binary star separation\
distance (in parsecs)? Recommended is 0.07 (based on Alpha Centuri): "))"""

# Filtering function for stars within 6.5 parsecs
...
# Using Astropy's SkyCoord to calculate Cartesian coordinates
coords = SkyCoord(
    ra=filtered_data_ra * u.deg,  # RA in degrees
    dec=filtered_data_dec * u.deg,  # DEC in degrees
    distance=filtered_distances * u.pc,  # Distance in parsecs
    frame='icrs')
...
# Code for converting sformattars to star_id, dist, x,y,z 
converted_stars = []
...
# Combine Gaia stars with the processed additional stars
combined_stars = []
...
# Perform Binary Star Separation Filtering
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if separation < binary_separation_distance:
                to_remove.add(j)  # Remove one star in the binary pair
...
# Apply the binary separation filter
...
def print_shortest_path_info(G, path, target_id):
    if path:
        print(f"Shortest path from the Sun (ID 504) to Star ID {target_id}: {path} with distance {distance:.2f} parsecs")
    else:
